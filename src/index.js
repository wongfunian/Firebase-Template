...
import { onAuthStateChanged, signOut } from 'https://www.gstatic.com/firebasejs/9.6.3/firebase-auth.js';
...
function main(user) {
    ...
    newPostButton.addEventListener('click', () => {
        ...
        try {
            // creates a new post
            createPost({ avatarUrl: photoURL, uid, name: displayName, email }, newPostTextarea.value);
...
async function renderPosts() {
    ...
    if (posts.length === 0) {
        ...
        posts.sort((a, b) => b.createdAt.toDate() - a.createdAt.toDate()).forEach((post, i) => postsContainer.appendChild(Post({ post, renderPosts }, i !== 0 ? 'mt-4' : '')));
    }
}
